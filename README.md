...
```toml
probe = 'T=/opt/homebrew/bin/tmux; b=$(basename "$PWD" | tr .: __); "$T" has-session -t "=$b" 2>/dev/null || "$T" -L agentmux-frame has-session -t "=$b-frame" 2>/dev/null'
```

so the dot shows whether its amux session is alive — the agent on tmux's default socket, **or** a lingering frame on the `agentmux-frame` socket (so the dot stays lit if the agent exits but the frame wrapper is still up; a plain bare-amux without a frame only ever has the agent leg). `probe_interval` controls the cadence (`0` = check on focus/hot-reload only). Two socket notes: the **agent** check takes no `-L` (amux's agents live on tmux's default socket), while the **frame** check names the `agentmux-frame` socket. And warden imports your **login-shell PATH** at startup, so a bare `tmux` resolves even from a Finder/Dock launch (which otherwise gives a `.app` a minimal PATH without `/opt/homebrew/bin`) — the example uses an absolute path anyway as the robust choice, since it can't be tripped up by a PATH set only in interactive-only shell config; adjust it to your `which tmux` or simplify to a bare `tmux` as you prefer.

A tab's optional `kill` command (same cascade, same PATH notes) severs the session the dot represents: click the cyan dot once to arm, click again to confirm, and warden runs `kill` fire-and-forget — the surface stays open, and the probe re-runs immediately to update the dot. With agentmux, set it to `'"$HOME/.agentmux/bin/amux" --kill'` (cwd = the tab dir): that tears down the **whole project** — the agent session plus its frame and scratch terminal — matching everything the probe detects, where a raw `tmux kill-session` would kill only the agent and orphan the frame. Since the control lives on the presence dot, `kill` only does anything on a tab that also sets `probe` (no probe ⇒ no dot to click).
