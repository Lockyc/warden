...
- **Pop-out tabs** — pop the active tab into its own banner-only window with **⌘⇧O** (or the ⤢ control on its row). It's **session-preserving**: the live terminal, its scrollback, and the running process move across untouched — no restart. Closing the popped-out window returns the tab to where it came from (reopening its origin window first if you'd closed it), and a ⇱ control pops it back in from the sidebar. A split tab pops out with both terminals, side by side at the same divider ratio it had, with the same divider, ✕ and focus ring — the pane UI is identical docked or popped out.
- **Splits** — a `split` table (global, per-window, per-root, or per-tab) opens a tab already split, second pane on either side, at a declared size, running a declared command beside the tab's own. A tab with no config split still gets one at runtime with **⌘D** (or Tab ▸ Split). Either way you get a draggable divider between the two terminals; close a split from the **✕** on the divider, or just type `exit` in the second shell. A runtime split — and its ratio — is remembered per tab across restarts; a config split's dragged ratio holds only until the tab's terminals relaunch (unload, hot-reload, or app restart), then it snaps back to the declared size. **⌘W** and **⌘⇧O** always act on the whole tab, not just the focused pane: unloading drops both terminals, popping out takes both with it.
