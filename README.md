...
so the dot shows whether its amux session is alive. `probe_interval` controls the cadence (`0` = check on focus/hot-reload only). One thing you must get right, and one that's now handled for you: **don't pass `-L`** (amux's agent sessions live on tmux's default socket, so a custom socket wouldn't find them) — and warden imports your **login-shell PATH** at startup, so a bare `tmux` resolves even from a Finder/Dock launch (which otherwise gives a `.app` a minimal PATH without `/opt/homebrew/bin`). The example uses an absolute path anyway, which stays the robust choice — it can't be tripped up by a PATH that's set only in interactive-only shell config — so adjust it to your `which tmux` or simplify to a bare `tmux` as you prefer.

A tab's optional `kill` command (same cascade, same PATH and no-`-L` notes) severs the session the dot represents: click the cyan dot once to arm, click again to confirm, and warden runs `kill` fire-and-forget — the surface stays open, and the probe re-runs immediately to update the dot. Since the control lives on the presence dot, `kill` only does anything on a tab that also sets `probe` (no probe ⇒ no dot to click).
