...
- **Splits** — **⌘D** (or Tab ▸ Split) gives the active tab a second terminal beside its first, with a draggable divider between them. Close it from the **✕** on the divider, or just type `exit` in the second shell. The split — and its ratio — is remembered per tab across restarts. **⌘W** and **⌘⇧O** always act on the whole tab, not just the focused pane: unloading drops both terminals, popping out takes both with it.
