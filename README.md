...
- **Persistent, not last-window-quit** — warden opens the windows you mark `open_on_start` (default all) and shows a **window launcher** home when none are open, listing every configured window so you can raise or reopen one with a click. It's a persistent app: closing the last window never quits it — **⌘Q** does.
