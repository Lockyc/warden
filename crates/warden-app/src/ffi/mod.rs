...
// Action tag discriminants we handle, GENERATED by build.rs from the vendored `ghostty.h`.
// `ghostty_action_tag_e` is a plain C enum with no explicit values, so each tag equals its 0-based
// position — and upstream inserts members mid-list, since libghostty's embedding API is officially
// unstable. Read `tag` as a u32 and COMPARE (never transmute into a Rust enum): an unknown value
// from a future libghostty is then just "unhandled", not invalid-discriminant UB.
//
// FOOTGUN (cost one silent regression): these were hand-transcribed, and a revendor that inserted
// `CHECK_FOR_UPDATES` mid-enum shifted every later tag by one. Nothing failed — the `const _`
// size_of asserts guard struct LAYOUT, not discriminants — so clicking a link silently did nothing
// (real OPEN_URL matched no arm), a dying child decoded as OPEN_URL and read a garbage (ptr, len),
// and a progress report decoded as SHOW_CHILD_EXITED and unloaded the tab. Never re-inline these
// numbers; the header is the one source of truth, and build.rs re-derives them on every revendor.
include!(concat!(env!("OUT_DIR"), "/action_tags.rs"));
