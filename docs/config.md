...
**`open_tabs_section` pins an "Open" section above the main list** — a mirror row for every tab that currently has a terminal standing: a live surface, or one popped out into its own window. It resolves to `Config.open_tabs_section: bool` (global, no cascade; default false) and rides the chrome DTO per window (`InitDto.open_tabs_section`) on init and hot-reload refresh, so a live flip applies without a relaunch — a global-only edit is an empty reconcile, so the reload hook compares it explicitly and re-pushes every snapshot (see CLAUDE.md's global-config footgun). **chrome-core** renders the section (`openSection` on its DTO): the mirrored tab keeps its own row in its group or tree as well, so the main list never shuffles as terminals come and go, and mirrors are excluded from ⌘1–9 and tab cycling so neither the indices nor a cycle lap change. A session probe is deliberately *not* openness — a cold tab with a live `amux` session behind it stays out of the section.
