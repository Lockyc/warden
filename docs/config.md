...
**`kill` and `suspend` are the two ways to end a tab's session** — ☠ destroy and ⏻ suspend on the presence dot's confirm row. Each cascades and resolves identically to `probe` (`""` opts a level out → `Tab.kill`/`Tab.suspend: Option<String>`), independently of the other. Clicking the cyan dot arms the row; confirming either action runs that command **fire-and-forget** via `sh -c` (cwd = tab dir; `{dir}`/`{title}` substituted raw — quote them if paths may contain spaces) **and unloads the tab** (same cold state as ⌘W). A tab with only one of the two shows the other disabled on the armed row. `end_session` runs the command then bumps the window so the scheduler fast-bursts; the chrome does **not** optimistically clear the dot — it tracks `warden:session-state` alone, dropping once, with no flicker (see `docs/probing.md` and CLAUDE.md's kill-flicker footgun). A genuinely-failed command leaves the session present, so the dot correctly stays lit. The arm-state lives in the chrome only — no IPC until confirm. Because the controls live **on** the presence dot, both are only reachable on tabs that also set `probe` — no `probe` ⇒ no cyan dot ⇒ nothing to arm (the commands still resolve, they just render no affordance). This coupling is by design (documented, not enforced). Both inherit the same probe PATH/env footgun (half-closed by `restore_login_path` — see CLAUDE.md's probe-env footgun): absolute paths stay the robust choice, since warden imports PATH only, not your shell exports. The canonical pair is `kill = '"$HOME/.agentmux/bin/amux" --kill'` and `suspend = '"$HOME/.agentmux/bin/amux" --suspend'`, mirrors of `amux --probe`: with cwd = the tab dir each tears down the **whole project** — the agent session plus its `-frame`/`-term` sessions — so it reaps exactly what the probe detects. `--kill` ends the agent tabs for good; `--suspend` leaves them recorded as open, so the probe exits 3, the dot ghosts, and activating the tab types `cmd`, whose restore picker brings them back (only a tab that has had a conversation is restorable — an agent never prompted has nothing to resume). (amux itself shells bare `tmux`; same login-PATH resolution.)
