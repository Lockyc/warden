...
open_tabs_section = false              # optional; default false — pin an "Open" section above the main
                                       #   list mirroring every tab that is open (a live terminal, or one
                                       #   popped out); each mirrored tab keeps its own row as well.
                                       #   Cascades global → window (set it per `[[window]]` to override)
remember_tabs = false                  # optional; default false — a window respawns the tabs it last
                                       #   had loaded and reselects its active tab. Cascades global → window
...
kill  = "…"                            # optional; session-kill (☠ destroy) command, cascades global→window→tab; run fire-and-forget on confirm via the cyan dot
suspend = "…"                          # optional; session-suspend (⏻, restorable) command, cascades exactly like kill
...
[split]                                # optional; declare every tab's SECOND pane — cascades global→window→tab
side = "right"                         #   "left" | "right" (default right); where the second pane sits
size = 0.5                             #   the second pane's share of the hole, 0.1–0.9 (default 0.5)
cmd  = ""                              #   typed into the second pane's shell; absent/"" = bare scratch shell
...
open_tabs_section = true               # optional; per-window override of the global "Open" section flag
remember_tabs = true                   # optional; per-window override of the global remember_tabs flag
...
suspend = "…"                          # optional; session-suspend command override for this window
split  = false                         # optional; a whole `[window.split]` table, or false to opt this window out
  ...
  suspend    = "…"                     # optional; session-suspend command override for this tab ("" = opt out)
  split      = true                    # optional; a whole `[window.tab.split]` table, `true` = the default split, `false` = opt out
  ...
  suspend = "…"                        # optional; per-root suspend override ("" = opt out)
  split = false                        # optional; a whole `[window.root.split]` table for discovered projects, or false
```
...
**`kill` and `suspend` are the two ways to end a tab's session** — ☠ destroy and ⏻ suspend on the presence dot's confirm row. Each cascades and resolves identically to `probe` (`""` opts a level out → `Tab.kill`/`Tab.suspend: Option<String>`), independently of the other. Clicking the cyan dot arms the row (so does **⌘⇧K**, Tab ▸ Terminate Session…, whose second press confirms ☠; **⌘⇧S** suspends without arming); confirming either action runs that command **fire-and-forget** via `sh -c` (cwd = tab dir; `{dir}`/`{title}` substituted raw — quote them if paths may contain spaces) **and unloads the tab** (same cold state as ⌘W). A tab with only one of the two shows the other disabled on the armed row. `end_session` runs the command then bumps the window so the scheduler fast-bursts; the chrome does **not** optimistically clear the dot — it tracks `warden:session-state` alone, dropping once, with no flicker (see `docs/probing.md` and CLAUDE.md's kill-flicker footgun). A genuinely-failed command leaves the session present, so the dot correctly stays lit. The arm-state lives in the chrome only — no IPC until confirm. The mouse route lives **on** the presence dot, so on a tab without `probe` (no cyan dot) only ⌘⇧S / ⌘⇧K reach them. Both inherit the same probe PATH/env footgun (half-closed by `restore_login_path` — see CLAUDE.md's probe-env footgun): absolute paths stay the robust choice, since warden imports PATH only, not your shell exports. The canonical pair is `kill = '"$HOME/.agentmux/bin/amux" --kill'` and `suspend = '"$HOME/.agentmux/bin/amux" --suspend'`, mirrors of `amux --probe`: with cwd = the tab dir each tears down the **whole project** — the agent session plus its `-frame`/`-term` sessions — so it reaps exactly what the probe detects. `--kill` ends the agent tabs for good; `--suspend` leaves them recorded as open, so the probe exits 3, the dot ghosts, and activating the tab types `cmd`, whose restore picker brings them back (only a tab that has had a conversation is restorable — an agent never prompted has nothing to resume). (amux itself shells bare `tmux`; same login-PATH resolution.)

The confirm row is bound to the **cyan** dot only — a ghost is a drop on a *dead* server, so
there is nothing to end, and chrome-core never arms it.

**`split` declares a tab's second pane, and cascades as a whole table.** Global `[split]`, `[window.split]`, `[window.root.split]`, `[window.tab.split]` — the nearest set level wins **whole** (a tab table that sets only `size` takes the table defaults for `side` and `cmd`, not the global table's values); `split = false` at a nearer level opts out, `split = true` is the default table (right, `0.5`, bare shell). Resolution collapses it to `Tab.split: Option<Split>` (`resolve_split_level` + `cascade_split` in `resolve.rs`; `DEFAULT_SPLIT_SIZE`, `SPLIT_SIZE_MIN`/`MAX` are the defaults and the validated band, shared with the chrome's drag clamp). The second pane runs in the tab's own `shell` and `dir`; `split.cmd` is typed into it exactly like `cmd` (absent or `""` = bare shell). `probe`/`kill`/attention/presence stay on the primary. `size` is the **second** pane's share. On hot-reload a `side`/`size` change is a live re-layout (`set_meta`); adding/removing the split or changing its `cmd` respawns the tab (`respawn_tabs`). Runtime behaviour — the in-memory drag override, an exited pane staying closed until relaunch, ⌘D recreating it — is in `docs/native-splits-direction.md`.
...
**`open_tabs_section` pins an "Open" section above the main list** — a mirror row for every tab that currently has a terminal standing: a live surface, or one popped out into its own window. It **cascades global → window** (there is no tab level: the section is a whole-sidebar affordance and a sidebar belongs to a window), resolving to `Window.open_tabs_section: bool` — the nearest set level wins and both default false. A bool needs no `""` opt-out: `false` at either level *is* the opt-out. Because resolve collapses the cascade into the window, even a **global-only** edit surfaces as an ordinary per-window diff (`WindowUpdate.open_tabs_section`), so it hot-reloads through the normal reconcile path with no global-compare special case — unlike `density`/`sidebar_drag`, which need one. The flag rides the chrome DTO per window (`InitDto.open_tabs_section`) on init and refresh, and the `Update` op carries it chrome-only: no surface is touched. **chrome-core** renders the section (`openSection` on its DTO): the mirrored tab keeps its own row in its group or tree as well, so the main list never shuffles as terminals come and go, and mirrors are excluded from ⌘1–9 and tab cycling so neither the indices nor a cycle lap change. The two lists **scroll independently**: the section is content-sized up to 2/5 of the window and then scrolls within itself, so paging through a long project list never pushes the open tabs off the top (the ceiling can't strand space — every tab in the section is also a row in the main list). While the section is showing, the main list below it is de-emphasised (desaturated + dimmed) until the pointer enters it — with the open tabs pinned above, that list is the index you reach into rather than where you are working. A session probe is deliberately *not* openness — a cold tab with a live `amux` session behind it stays out of the section.

**`remember_tabs` brings a window back the way it was left** — whenever the window is built (launch, or a reopen), every tab that last had a terminal standing (live, or popped out) spawns eagerly as if `load_on_open`, and the tab that was active is reselected (else the first tab, as usual). It **cascades global → window** like `open_tabs_section`, resolving to `Window.remember_tabs: bool`, default false. `reconcile` does not diff it: the app reads it from the current config when it builds a window and when it records one, so a flip takes effect at the next record — turning it off forgets the window's record. Records are keyed by window label and tab identity (`Tab::key`), so a renamed window starts fresh and a tab that has left the config is skipped; a popped-out tab comes back docked. The app's `session.rs` owns the store and `WindowManager::record_session` its write rule (never while quitting).

Validation: unique window title, unique tab **identity** (`id`-else-normalized-`dir`) within a window (window-wide — across loose tabs and all groups, curated tabs only; two curated tabs sharing a `dir` with no disambiguating `id` → `DuplicateTabIdentity` — see CLAUDE.md's tab-identity footgun; `title` carries no uniqueness constraint), non-empty title/dir/explicit-title, non-empty root dir/name, root `depth >= 1`, malformed colour → **errors**; width/height ≤ 0 → **error**; section-name uniqueness (group names + root names) is one shared namespace **window-wide**: a group-vs-group clash is `DuplicateGroup`, any other clash (root-vs-root or a root sharing a group's name) is `DuplicateSection`; a `dir` that doesn't exist (tab or root) → **warning** (tab/root still created, so a root can point at a not-yet-cloned dir without erroring); an unknown `split.side` or a `split.size` outside `SPLIT_SIZE_MIN..=SPLIT_SIZE_MAX` → `ResolveError` (validated per level, so a bad global table errors even with no tabs). Invalid config must be reported, never panic.

A tab whose `dir` is missing also renders a **⚠ marker** on its sidebar row. That marker is **derived per render** from the directory's existence right now (`Registry::tab_dtos`), never cached at materialize time — so declaring a tab for a repo you're about to clone is fine: the ⚠ clears on the next sidebar refresh once the directory appears, with no relaunch. (It is a marker, not a block — the tab still opens, and the shell reports the missing dir itself.)
